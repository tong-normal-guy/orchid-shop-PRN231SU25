...
{
    private readonly ILogger<CreateOrchidModel> _logger;
    private readonly OrchidApiService _orchidService;
    private readonly CategoryApiService _categoryService;

    public CreateOrchidModel(
        ILogger<CreateOrchidModel> logger, 
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        {
            _logger.LogError(ex, "Error loading create orchid page");
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        try
        {
            ...
            // Create the orchid
            var createResponse = await _orchidService.CreateOrchidAsync(Orchid);

            if (createResponse?.Success == true)
            {
                _logger.LogInformation("Successfully created new orchid: {Name}", Orchid.Name);
                TempData["SuccessMessage"] = "Orchid created successfully!";
                return RedirectToPage("/Admin/Orchids");
            }
            else
            {
                _logger.LogWarning("Failed to create orchid. Message: {Message}", createResponse?.Message);
                TempData["ErrorMessage"] = createResponse?.Message ?? "Failed to create orchid.";
                return Page();
            }
        }
        catch (Exception ex)
        {
            ...
            TempData["ErrorMessage"] = "An error occurred while creating the orchid. Please try again.";
            return Page();
        }
    }
